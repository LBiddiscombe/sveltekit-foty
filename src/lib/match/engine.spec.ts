...
describe('playGame', () => {
	it('returns a MatchResult with played=true', () => {
		const result = playGame(3, 5, ['goal', 'miss', 'goal'], 'Gillingham', 'Gillingham', true);
		expect(result.played).toBe(true);
		expect(result.chances).toBe(3);
	});

	it('counts player goals from outcomes', () => {
		const result = playGame(3, 5, ['goal', 'goal', 'miss'], 'Gillingham', 'Gillingham', true);
	...
	it('preserves outcome order', () => {
		...
		const result = playGame(5, 5, [...outcomes], 'Gillingham', 'Gillingham', true);
	...
	it('returns a copy of outcomes array', () => {
		...
		const result = playGame(2, 5, outcomes, 'Gillingham', 'Gillingham', true);
		outcomes.push('goal');
	...
	it('gives rating 7 for 1 player goal', () => {
		const result = playGame(3, 5, ['goal', 'miss', 'miss'], 'Gillingham', 'Gillingham', true);
		expect(result.rating).toBe(7);
	});

	it('gives rating 5 for 0 player goals', () => {
		const result = playGame(3, 5, ['miss', 'saved', 'miss'], 'Gillingham', 'Gillingham', true);
		expect(result.rating).toBe(5);
	});

	it('handles empty outcomes', () => {
		const result = playGame(0, 5, [], 'Gillingham', 'Gillingham', true);
...
describe('skipGame', () => {
	it('returns a MatchResult with played=false', () => {
		const result = skipGame(3, 5, 'Gillingham', 'Gillingham', true);
		expect(result.played).toBe(false);
	});

	it('returns empty outcomes (no chances taken)', () => {
		const result = skipGame(4, 5, 'Gillingham', 'Gillingham', true);
		expect(result.outcomes).toEqual([]);
	});

	it('has rating 4', () => {
		const result = skipGame(2, 5, 'Gillingham', 'Gillingham', true);
	...
	it('handles zero chances', () => {
		const result = skipGame(0, 5, 'Gillingham', 'Gillingham', true);
		expect(result.chances).toBe(0);
		expect(result.outcomes).toEqual([]);
	});
});
