...
**Team morale** — A team-level stat (1–10 scale, starts at 5, clamped) stored in the `season` store. Shifts the λ of the Poisson distribution used for team base goals: `λ = 0.3 + strength×0.1 + morale×0.01`. Opponent λ uses opponent club strength only: `λ = 0.3 + strength×0.1` (morale does not affect conceding). Higher morale → team scores more. Morale deltas: +1 for win, 0 for draw, −1 for loss, based purely on match result. Skipped matches still apply the result-based delta.

**Match result scoring** — Two-part model. (1) The **team base** generates goals via a Poisson distribution whose λ is determined by team morale (applies in both Play and Skip paths). (2) On the Play path, the player's minigame goals are **added** to the team base to form the final score. Opponent goals are Poisson-distributed from their own club's strength only (morale does not affect conceding). This means blowout scores are possible when the player performs well in minigames on top of a strong team baseline. No randomness in the Play path's minigame outcomes — those are pure skill.
